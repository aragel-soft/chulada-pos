...
fn get_sequence(conn: &Connection) -> Result<i64, String> {
    let last_folio: Option<String> = conn
        .query_row(
...
fn get_relevant_kit_rules(
    ...
    let ids_placeholder: String = product_ids
        .iter()
        .map(|_| "?")
        .collect::<Vec<_>>()
        .join(",");
    ...
    let kits_placeholder: String = kit_ids_vec
        .iter()
        .map(|_| "?")
        .collect::<Vec<_>>()
        .join(",");
...
fn apply_kit_rules(
    conn: &Connection,
    items: &[SaleItemRequest],
) -> Result<Vec<SaleItemRequest>, String> {
    // Get Kit Rules
    ...
    // Auto-Link Triggers
    let active_kit_ids: HashSet<String> = items
        .iter()
        .filter_map(|i| i.kit_option_id.clone())
        .collect();
...
fn validate_promotions(
    ...
    let mut params: Vec<&dyn rusqlite::ToSql> = promo_ids
        .iter()
    ...
    let combos_iter = stmt_combos
        .query_map(rusqlite::params_from_iter(promo_ids.iter()), |row| {
            Ok((
    ...
    for (promo_id, promo_items) in promo_groups {
        ...
        let required_products = promo_combos.get(&promo_id).ok_or(format!(
        ...
        let first_provided_qty = match provided_products.get(first_required_prod_id) {
            ...
            None => {
                return Err(format!(
                    "Promoción '{}': falta el producto '{}'",
                    promo_id, first_required_prod_id
                ))
            }
        };
...
fn validate_credit_sale(
    ...
    let sale_type = if payment_method == "credit" {
        "credit"
    } else {
        "cash"
    };
    ...
    if sale_type == "credit" {
        ...
        // Check Limit & Balance
        let (current_balance, credit_limit, _customer_name): (f64, f64, String) = tx
            .query_row(
...
fn calculate_sale_items<'a>(
    ...
    for (promo_id_opt, group_items) in promo_groups {
        if let Some(promo_id) = promo_id_opt {
            // PROMO ITEMS
            let validated_promo = validated_promos
                .get(&promo_id)
            ...
            for (item, item_retail) in item_retails {
                let (db_code, db_name, _, _) = products_map
                    .get(&item.product_id)
                    .ok_or(format!("Producto {} no encontrado", item.product_id))?;

                let proportion = if total_retail > 0.0 {
...
pub fn process_sale(
    ...
    let (total_gross, total_item_discounts, final_items) =
        calculate_sale_items(&tx, &validated_items, payload.discount_percentage)?;
    ...
    if let Some(code) = &payload.voucher_code {
        ...
        let max_applicable = if v_balance > final_total {
            final_total
        } else {
            v_balance
        };
    ...
    if total_paid < final_total - 0.01 {
        return Err(format!(
            "Pago insuficiente. Total calculado: ${:.2}, Pagado: ${:.2} (Incluye ${:.2} de vale)",
            final_total, total_paid, voucher_amount_used
        ));
    }

    // Credit Validation
    let customer_id_opt = validate_credit_sale(
        &tx,
    ...
    tx.execute(
        "INSERT INTO sales (
            id, folio, sale_date, subtotal, discount_percentage, discount_amount, total,
    ...
    for (i, data) in final_items.iter().enumerate() {
        ...
        if rows_mod == 0 {
            return Err(format!(
                "Producto no encontrado en inventario (o sin stock inicializado): {}",
                data.db_name
            ));
        }
        ...
        tx.execute(
            "INSERT INTO sale_items (
                ...
                subtotal, kit_option_id, promotion_id, total, created_at
    ...
    if let Some(v_id) = voucher_id_used {
        if voucher_amount_used > 0.0 {
            tx.execute(
                "UPDATE store_vouchers SET current_balance = current_balance - ?1, updated_at = ?2 WHERE id = ?3",
                params![voucher_amount_used, now_local, v_id]
    ...
    // Calculate change
    let change = if total_paid > final_total {
    ...
    if payload.should_print {
        ...
        tauri::async_runtime::spawn_blocking(move || {
            if let Err(_e) =
                crate::printer_utils::print_sale_from_db(app_handle_clone, sale_id_clone)
            {
            }
        });
    } else {
