...
pub struct KitListItem {
    ...
    triggers_count: i64,      // Campo calculado: Cuántos productos activan este kit
    items_summary: Option<String>, // Campo calculado: Texto resumen (ej: "1x Tinte + 1x Peróxido")
    is_active: bool,
}

// Reutilizamos la estructura de paginación estándar
#[derive(Serialize)]
...
pub fn get_kits(
  ...
  let base_sql = "
    SELECT 
      ...
      k.description,
      k.is_active,
      -- Subconsulta 1: Contar cuántos triggers existen
      (SELECT COUNT(*) FROM product_kit_main WHERE kit_option_id = k.id) as triggers_count,
      -- Subconsulta 2: Concatenar nombres y cantidades de los regalos
      (
  ...
  let final_sql = if has_search {
    format!("{} AND k.name LIKE ?1 ORDER BY k.created_at DESC LIMIT ?2 OFFSET ?3", base_sql)
  } else {
    format!("{} ORDER BY k.created_at DESC LIMIT ?1 OFFSET ?2", base_sql)
  };
