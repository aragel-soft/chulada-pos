...
fn validate_kit_instances(
    ...
    let placeholders = sale_item_ids
        .iter()
        .map(|_| "?")
        .collect::<Vec<_>>()
        .join(",");
    ...
    let rows = stmt
        .query_map(rusqlite::params_from_iter(sale_item_ids.iter()), |row| {
            Ok((
    ...
    let available_rows = stmt_available
    ...
    for row_result in available_rows {
        ...
        let available_qty = available_quantities
            .get(&sale_item_id)
            .copied()
            .unwrap_or(0.0);
        *available_by_kit
            ...
            .or_insert_with(HashMap::new)
            .entry(product_id)
            .or_insert(0.0) += available_qty;
    }

    for (kit_id, return_products) in kit_groups {
        let kit_rule = kit_rules
            .get(&kit_id)
        ...
        for (product_id, qty) in &return_products {
            if kit_rule.triggers.contains(product_id) {
                ...
                return Err(format!(
                    "El producto '{}' no pertenece al kit '{}'",
                    product_id, kit_rule.name
                ));
            }
        }
        ...
        let available_kit_products = available_by_kit.get(&kit_id).ok_or(format!(
            "Kit '{}' no tiene productos disponibles para devolver",
            kit_rule.name
        ))?;
        ...
        if total_trigger_av < 0.001 {
            return Err(format!(
                "No hay productos principales disponibles para devolver del kit '{}'",
                kit_rule.name
            ));
        }
...
fn validate_promotion_instances(
    ...
    for (promo_id, return_products) in return_promo_groups {
        let available_products = available_by_promo.get(&promo_id).ok_or(format!(
            "Promoción '{}' no tiene productos disponibles para devolver",
            promo_id
        ))?;

        let first_return_product_id = return_products.keys().next().ok_or(format!(
            "Promoción '{}' no tiene productos en la devolución",
            promo_id
        ))?;

        let available_qty = available_products
            .get(first_return_product_id)
            .ok_or(format!(
                "Producto '{}' no disponible en la promoción",
                first_return_product_id
            ))?;
        ...
        if *available_qty < 0.001 {
            return Err(format!(
                "No hay cantidades disponibles para devolver de la promoción '{}'",
                promo_id
            ));
        }
...
fn update_sale_status(tx: &Connection, sale_id: &str) -> Result<(), String> {
    ...
    for (_, (original, returned)) in &sale_items_map {
        ...
        if (*original - *returned).abs() > 0.001 {
...
pub fn process_return(
    ...
    if payload.reason.trim().is_empty() {
        return Err("Debe especificar un motivo de devolución".to_string());
    }
    ...
    for item in &payload.items {
        if item.quantity <= 0.0 {
            return Err(format!("Cantidad debe ser > 0 para {}", item.product_id));
        }
    }
...

    {
        ...
        let mut params: Vec<&dyn rusqlite::ToSql> = item_ids
            .iter()
    ...
    for item in &payload.items {
        let (orig_qty, name, code, pid, db_total) = sale_items_data
            .get(&item.sale_item_id)
            .ok_or(format!("Item no encontrado: {}", item.sale_item_id))?;

        let returned_so_far = already_returned_map
            .get(&item.sale_item_id)
            .copied()
            .unwrap_or(0.0);
        ...
        if (returned_so_far + item.quantity) > (orig_qty + 0.001) {
            return Err(format!(
                "Exceso de devolución para '{}'. Disp: {:.2}, Solicitado: {}",
                name, available, item.quantity
            ));
        }

        let real_unit_price = if *orig_qty > 0.0001 {
            db_total / orig_qty
        } else {
            0.0
        };
    ...
    let return_total: f64 = validated_items
        .iter()
        .map(|(i, _, _, _)| i.unit_price * i.quantity)
        .sum();
    if return_total <= 0.0 {
        return Err("El total de devolución debe ser > 0".to_string());
    }
