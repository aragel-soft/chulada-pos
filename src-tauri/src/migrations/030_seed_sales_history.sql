...
SELECT 
  'sale-001', 'XAL1-00001', datetime('now', 'localtime'), 130.00, 130.00, 'completed', 'cash', 130.00, user_id, 0
FROM _vars;
...
SELECT 
  'sale-002', 'XAL1-00002', datetime('now', '-1 day', 'localtime'), 315.00, 315.00, 'completed', 'card_transfer', 315.00, user_id, 0
FROM _vars;
...
SELECT 
  'sale-003', 'XAL1-00003', datetime('now', '-2 days', '12:00:00'), 2800.00, 2800.00, 'completed', 'cash', 2800.00, user_id
FROM _vars;
...
SELECT 
  'sale-004', 'XAL1-00004', datetime('now', '-7 days'), 180.00, 180.00, 'cancelled', 'cash', user_id, 'Cliente olvidó la cartera', datetime('now', '-7 days', '+10 minutes')
FROM _vars;
...
SELECT 
  'sale-005', 'XAL1-00005', datetime('now', '-1 day', '16:30:00'), 1000.00, 10.00, 100.00, 900.00, 'completed', 'mixed', 400.00, 500.00, 1, user_id
FROM _vars;
...
INSERT INTO sales (id, folio, sale_date, subtotal, total, status, payment_method, cash_amount, user_id)
SELECT 
  'sale-006', 'XAL1-00006', datetime('now', 'localtime'), 120.00, 120.00, 'completed', 'cash', 120.00, user_id
FROM _vars;
...
INSERT INTO sale_items (id, sale_id, product_id, product_name, product_code, quantity, unit_price, price_type, subtotal) VALUES
('si-006-a', 'sale-006', 'p001', 'Tinte Kuul Rubio (Pack)', 'KUL-902', 1, 50.00, 'promo', 50.00), -- Bajó de 55 a 50
('si-006-b', 'sale-006', 'p005', 'Peróxido Hidra (Pack)', 'HID-PER', 1, 70.00, 'promo', 70.00); -- Bajó de 85 a 70
...
INSERT INTO sales (id, folio, sale_date, subtotal, total, status, payment_method, cash_amount, user_id)
SELECT 
  'sale-007', 'XAL1-00007', datetime('now', '-3 days'), 45.00, 45.00, 'completed', 'cash', 45.00, user_id
FROM _vars;
