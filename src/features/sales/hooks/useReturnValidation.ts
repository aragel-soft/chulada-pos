        ...
        if (originalMainQty > 0) {
          ...
          if (totalMainQty > 0 && totalGiftQty === 0) {
            if (expectedGiftQty > 0) {
              ...
              messages.push(
                `Para devolver productos principales (${mainNames}), debes devolver ${expectedGiftQty} complemento(s) del kit.`
              );
            }
          } else if (totalGiftQty > 0 && totalMainQty === 0) {
            messages.push(
              "No puedes devolver solo complementos sin devolver el producto principal del kit."
            );
            ...
            if (Math.abs(totalGiftQty - expectedGiftQty) > 0.001) {
              ...
              messages.push(
                `Para devolver ${mainNames}, debes devolver exactamente ${expectedGiftQty} complemento(s) (actualmente: ${totalGiftQty}).`
              );
            }
          }
        }
