...
CREATE TABLE IF NOT EXISTS product_kit_options (
  ...
  name TEXT NOT NULL,
  description TEXT,
  -- is_required: Si es 1, el sistema debe obligar/agregar el complemento.
  is_required BOOLEAN DEFAULT 1, 
  -- max_selections: Por defecto 1 para cumplir regla "1 a 1" (un complemento por compra).
...
-- 2. Items (Lo que se lleva como complemento o incluido)
CREATE TABLE IF NOT EXISTS product_kit_items (
  id TEXT PRIMARY KEY,
  kit_option_id TEXT NOT NULL,
  included_product_id TEXT NOT NULL,
  -- quantity: Por defecto 1. Define cuántas unidades de ESTE producto específico se incluyen como complemento.
...
-- 3. Productos Principales (Productos que activan/disparan la regla)
...
-- Índices
-- idx_one_kit_per_product: Garantiza que un producto principal solo active un kit a la vez.
